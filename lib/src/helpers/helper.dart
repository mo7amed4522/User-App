...
class Helper {
  ...
  static nextOpenTime(Vendor vendorList){
    ...
    if(availableType=='1'){
      ...
      if(status==true) {
        ...
        next =  '${vendorList.shopTiming.allTimes.fromAvailableTime}-${vendorList.shopTiming.allTimes.toAvailableTime}';
      }
    } else if(availableType=='2'){
      try {
  ...
  static pricePrint(String amount) {
   ...
   // currency = roundOffToXDecimal(displayAmount,numberOfDecimal: setting.value.currencyDecimalDigits);
    ...
    currency = double.parse(double.parse(amount).toStringAsFixed(setting.value.currencyDecimalDigits));
    RegExp regex = RegExp(r'([.]*0)(?!.*\d)');
    displayCurrency = currency.toString().replaceAll(regex, '');
    /* switch (monenytype) {
      case 1:
    ...
    if (setting.value.currencyRight) {
      return '$displayCurrency ${setting.value.defaultCurrency}';
    } else {
      return '${setting.value.defaultCurrency} $displayCurrency';
